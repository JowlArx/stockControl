...
router.post('/', (req, res) => {
    ...
    if (!product_id || !stock_quantity || !location) {
        return res.status(400).send('Los campos "product_id", "stock_quantity" y "location" son obligatorios');
    }

    const query = `
        INSERT INTO inventory (product_id, stock_quantity, location, updated_at)
        VALUES (?, ?, ?, NOW())
    `;

    db.query(query, [product_id, stock_quantity, location], (err, result) => {
        if (err) {
...
// Actualizar un registro de inventario (PUT)
router.put('/:id', (req, res) => {
    ...
    const query = `
        UPDATE inventory
        SET product_id = ?, stock_quantity = ?, location = ?, updated_at = NOW()
        WHERE id = ?
    `;

    db.query(query, [product_id, stock_quantity, location, id], (err, result) => {
        if (err) {
